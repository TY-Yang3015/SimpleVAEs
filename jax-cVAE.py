...
class ConvEncoder(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
        x = hk.Conv2D(output_channels=32, kernel_shape=4, stride=2, padding="SAME")(x)
        x = jax.nn.relu(x)
        x = hk.Conv2D(output_channels=64, kernel_shape=4, stride=2, padding="SAME")(x)
        x = jax.nn.relu(x)
        x = hk.Conv2D(output_channels=128, kernel_shape=4, stride=2, padding="SAME")(x)
        x = jax.nn.relu(x)
        x = hk.Conv2D(output_channels=256, kernel_shape=4, stride=2, padding="SAME")(x)
        x = jax.nn.relu(x)
        x = hk.Flatten()(x)
        mean = hk.Linear(self.latent_size)(x)
        log_stddev = hk.Linear(self.latent_size)(x)
        return mean, jnp.exp(log_stddev)
...
class ConvDecoder(hk.Module):
    ...
    def __call__(self, z: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
        z = hk.Linear(32*32*16)(z)  # Adjust the size according to your architecture
        z = jax.nn.relu(z)
        z = jnp.reshape(z, (-1, 32, 32, 16))
        z = hk.Conv2DTranspose(output_channels=128, kernel_shape=4, stride=2, padding="SAME")(z)
        z = jax.nn.relu(z)
        z = hk.Conv2DTranspose(output_channels=64, kernel_shape=4, stride=2, padding="SAME")(z)
        z = jax.nn.relu(z)
        z = hk.Conv2DTranspose(output_channels=32, kernel_shape=4, stride=2, padding="SAME")(z)
        z = jax.nn.relu(z)
        mu = hk.Conv2DTranspose(output_channels=self.output_shape[-1], kernel_shape=4, stride=2, padding="SAME")(z)
        log_var = hk.Conv2DTranspose(output_channels=self.output_shape[-1], kernel_shape=4, stride=2, padding="SAME")(z)
        return mu, log_var
